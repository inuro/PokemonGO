...
FILE_MOVE=$GAMEMASTERDIR/move.tsv
FILE_MOVE_COMBAT=$GAMEMASTERDIR/move_combat.tsv
FILE_SHADOW_MOVE=$GAMEMASTERDIR/shadow_purified_move.tsv

#files statically prepared in sidefiles directory
FILE_LOCALIZE_POKEMON=$SIDEFILEDIR/localize_pokemon.tsv
FILE_LOCALIZE_MOVE=$SIDEFILEDIR/localize_move.tsv
...
-------------------------------------------------------------------------------
-- Shadow&Purified moves raw table "_shadow_move"
drop table if exists $TABLE_SHADOW_MOVE CASCADE;
create table if not exists $TABLE_SHADOW_MOVE(
    pokemon_id text,
    uid text,
    shadowChargeMove text,
    purifiedChargeMove text
);
\copy $TABLE_SHADOW_MOVE(pokemon_id, uid, shadowChargeMove, purifiedChargeMove) from '$FILE_SHADOW_MOVE' with CSV delimiter E'\t' NULL '';
...
-- SPECIAL treats for Shadow/Purified charge move
insert into $TABLE_MON_CHARGE_RAW 
select pokemon_id, concat(replace(uid,'_NORMAL',''), '_SHADOW') as uid, shadowChargeMove as move, true::boolean as legacy 
from $TABLE_POKEMON
left join $TABLE_SHADOW_MOVE using(pokemon_id, uid)
where shadow=true and uid!~'_SHADOW$'
;

insert into $TABLE_MON_CHARGE_RAW 
select pokemon_id, uid, purifiedChargeMove as move, true::boolean as legacy 
from $TABLE_POKEMON
left join $TABLE_SHADOW_MOVE using(pokemon_id, uid)
where shadow=true and uid!~'_SHADOW$'
;
...
-------------------------------------------------------------------------------
-- move data raw table
drop table if exists $TABLE_MOVE_RAW CASCADE;
create table if not exists $TABLE_MOVE_RAW(
    index text,
    uid text,
    type text,
    power int,
...
\copy $TABLE_MOVE_RAW(index,uid,type,power,duration,energy,damage_window_start,damage_window_end) from '$FILE_MOVE' with CSV delimiter E'\t' NULL '';

-------------------------------------------------------------------------------
-- Combat move data raw table
drop table if exists $TABLE_MOVE_COMBAT_RAW CASCADE;
create table if not exists $TABLE_MOVE_COMBAT_RAW(
    index text,
    uid text,
    type text,
    power int,
...
\copy $TABLE_MOVE_COMBAT_RAW(index,uid,type,power,duration,energy,buff,target_at,target_df,attacker_at,attacker_df) from '$FILE_MOVE_COMBAT' with CSV delimiter E'\t' NULL '';
...
-------------------------------------------------------------------------------
-- Move loalization
drop table if exists $TABLE_LOCALIZE_MOVE CASCADE;
create table if not exists $TABLE_LOCALIZE_MOVE(
    index text,
    uid text,
    jp text,
    en text
);
\copy $TABLE_LOCALIZE_MOVE(index, uid, en, jp) from '$FILE_LOCALIZE_MOVE' with CSV delimiter E'\t' NULL '';
...
\copy $TABLE_POKEMON_NOT_YET(pokemon_id, uid) from '$FILE_POKEMON_NOT_YET' with CSV delimiter E'\t' NULL 'null';
...
BEGIN
    ...
    if not is_empty(defender_type1) then 
        select mlp from $TABLE_EFFECTIVENESS_RAW where attacker=attacker_type and defender=defender_type1 into val1;
    end if;
    if not is_empty(defender_type2) then 
        select mlp from $TABLE_EFFECTIVENESS_RAW where attacker=attacker_type and defender=defender_type2 into val2;
    end if;
    return val1 * val2;
END
' LANGUAGE 'plpgsql';
...
CREATE INDEX idx_effectiveness ON $TABLE_EFFECTIVENESS_RAW USING btree(attacker,defender);
...
\copy $TABLE_POKEMON_NOT_YET(pokemon_id, uid) from '$FILE_POKEMON_NOT_YET' with CSV delimiter ',' NULL 'null';
-- SPECIAL treats for NIDORAN
UPDATE $TABLE_POKEMON_NOT_YET set uid='NIDORAN_F' where pokemon_id='29' and uid='NIDORAN';
UPDATE $TABLE_POKEMON_NOT_YET set uid='NIDORAN_M' where pokemon_id='32' and uid='NIDORAN';
...
create table if not exists $TABLE_POKEMON_FASTMOVE as (
    ...
    join $TABLE_MOVE_RAW C on C.index=B.move
...
create table if not exists $TABLE_POKEMON_FASTMOVE_COMBAT as (
    ...
    from $TABLE_FORM_RAW A 
    join $TABLE_MON_FAST_RAW B on B.pokemon_id=A.pokemon_id and coalesce(B.form_id,'') = coalesce(A.form_id,'')
    join $TABLE_MOVE_COMBAT_RAW C on C.index=B.move
    join $TABLE_TYPE_RAW D on D.index=C.type
    order by index
);
...
create table if not exists $TABLE_EFFECTIVENESS as (
    select 
        ...
        C.uid as defender, 
        A.mlp 
    from $TABLE_EFFECTIVENESS_RAW A 
...
DECLARE
    ...
    turn int:=1;
BEGIN
    select * into fast from $TABLE_MOVE_RAW F where F.uid=f_uid; 
    select * into charge from $TABLE_MOVE_RAW C where C.uid=c_uid; 
    LOOP
        -- FAST move
        hpt := hpt - f_dmg;
...
join $TABLE_LOCALIZE_MOVE LF on LF.uid=S.f_uid
join T_DPS on true
--join T_AC on true
;
return;
...
BEGIN
    IF name ~ ''^\w+$'' THEN
        select * into temp from $TABLE_LOCALIZE_MOVE where uid=UPPER(name)
    ELSE
        select * into temp from $TABLE_LOCALIZE_MOVE where jp=name
    END IF;
    return temp.uid;
END
' LANGUAGE 'plpgsql';
...
join localize_move C on C.uid=A.f_uid
order by A.index
);
