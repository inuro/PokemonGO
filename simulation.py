...
# usage for output
# 
# select B.jp,C.jp,D.jp,A.win,A.draw,A.lose  from simulation A left join localize_pokemon B using(uid) left join localize_fastmove C on C.uid=A.fast left join localize_chargemove D on D.uid=A.charge where cap=1500 order by win desc, lose limit 50;
...
# clear target records only
    ...
    for cap in caps:
        ...
        query = "DELETE from simulation where cap={} and shield={};".format(cap, shield)
        cur2.execute(query)
...
for cap in caps:
    ...
    for p1 in results:
        ...
        # insert simulation result into the table
        query = "INSERT into simulation(uid,fast,charge,win,draw,lose,cap,shield) values ('{}','{}','{}',{},{},{},{},{});".format(A['uid'],A['fast'],A['charge'], win, draw, lose, cap, shield)
        cur2.execute(query)
        conn.commit()
