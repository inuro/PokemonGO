...
db = 'sandbox'
table = 'simulation_halloween_ultra'
...
for cap in caps:
    ...
    query = "select A.* from pattern_combat A left join _pokemon B using(uid) where cap_cp={} and cap_lv={} and cp>1750 and (A.type_1 in ('BUG','POISON','DARK','FAIRY','GHOST') or A.type_2 in ('BUG','POISON','DARK','FAIRY','GHOST'));".format(cap_cp,cap_lv)
    cur1.execute(query)
