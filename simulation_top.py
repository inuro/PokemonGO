...
# usage for output
# 
# select B.jp,C.jp,D.jp,A.win,A.draw,A.lose  from simulation_top A left join localize_pokemon B using(uid) left join localize_fastmove C on C.uid=A.fast left join localize_chargemove D on D.uid=A.charge where cap=1500 order by win desc, lose limit 50;
...
parser.add_argument('-i', '--init', type=bool, default=False, help='Initialize simulation_top table')
...
if args.init == True:
    ...
    cur2 = conn.cursor()
    query = """
    DROP TABLE if exists simulation_top;
    CREATE TABLE simulation_top(
        uid text,
        fast text,
        charge text,
...
# clear target records only
    ...
    for cap in caps:
        ...
        query = "DELETE from simulation_top where cap={} and shield={};".format(cap, shield)
        cur2.execute(query)
...
for cap in caps:
    ...
    b as(
        select stddev(win-lose) as dev_win, avg(win-lose) as avg_win from a  
    ), 
    C as( 
        select A.*, (win-lose - avg_win) / dev_win * 10 + 50 as T_win from A left join B on true
    ), 
    D as (
        select uid, fast, charge, cap from C where T_win>60
    )
    select E.* from D
    left join pattern_combat E using(uid,fast,charge,cap) 
    ;
    """
    ...
    for p1 in results:
        ...
        # insert simulation_top result into the table
        query = "INSERT into simulation_top(uid,fast,charge,win,draw,lose,cap,shield) values ('{}','{}','{}',{},{},{},{},{});".format(A['uid'],A['fast'],A['charge'], win, draw, lose, cap, shield)
        cur2.execute(query)
        conn.commit()
