...
select 
    ...
    max(case when key='fastAttackBonusMultiplier' then value else 1.0 end) as fast_mlp,
    max(case when key='chargeAttackBonusMultiplier' then value else 1.0 end) as charge_mlp
...
left join _move C on c.uid=b.move 
left join _move E on E.uid=D.move
left join SETTINGS on true
where A.at is not null and C.uid is not null
and C.uid not in ('TRANSFORM_FAST','STRUGGLE')
