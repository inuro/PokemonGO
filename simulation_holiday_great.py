#!/var/bin/python
# $ python simulation_holiday_great.py
# ホリデーカップ：スーパーリーグバージョン：CP1,500まで＋ノーマル、くさ、でんき、こおり、ひこう、ゴーストタイプのポケモンのみ
# 
...
db = 'sandbox'
table = 'simulation_holiday_great'
...
for cap in caps:
    ...
    query = "select A.* from pattern_combat A left join _pokemon B using(uid) where cap_cp={} and cap_lv={} and cp>1200 and (A.type_1 in ('NORMAL','GLASS','ELECTRIC','ICE','FLYING','GHOST') or A.type_2 in ('NORMAL','GLASS','ELECTRIC','ICE','FLYING','GHOST'));".format(cap_cp,cap_lv)
    cur1.execute(query)
