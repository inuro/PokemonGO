...
# usage for output
# 
# select B.jp,C.jp,D.jp,A.win,A.draw,A.lose  from simulation_kanto A left join localize_pokemon B using(uid) left join localize_fastmove C on C.uid=A.fast left join localize_chargemove D on D.uid=A.charge where cap=1500 order by win desc, lose limit 50;
...
parser.add_argument('-i', '--init', type=bool, default=False, help='Initialize simulation_kanto table')
...
if args.init == True:
    ...
    cur2 = conn.cursor()
    query = """
    DROP TABLE if exists simulation_kanto;
    CREATE TABLE simulation_kanto(
        uid text,
        fast text,
        charge text,
...
# clear target records only
    ...
    for cap in caps:
        ...
        query = "DELETE from simulation_kanto where cap={} and shield={};".format(cap, shield)
        cur2.execute(query)
...
for cap in caps:
    ...
    query = "select A.* from pattern_combat A left join _pokemon B using(uid) where cap='{}' and pokemon_id::numeric<=151"
    ...
    for p1 in results:
        ...
        # insert simulation_kanto result into the table
        query = "INSERT into simulation_kanto(uid,fast,charge,win,draw,lose,cap,shield) values ('{}','{}','{}',{},{},{},{},{});".format(A['uid'],A['fast'],A['charge'], win, draw, lose, cap, shield)
        cur2.execute(query)
        conn.commit()
