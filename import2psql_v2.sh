...
#
#   [usage]
#   $ import2psql_v2.sh <DB name> <game_master dir> <sidefile dir>
#   [example]
#   $ ./import2psql_v2.sh pokemongo2 masterfiles sidefiles
#   
#   [note]
...
-------------------------------------------------------------------------------
-- Pokemon stats raw table "_stats"
-- 0001	BULBASAUR	GRASS	POISON	118	111	128
-- 0001	BULBASAUR_FALL_2019	GRASS	POISON	118	111	128
-- 0001	BULBASAUR_NORMAL	GRASS	POISON	118	111	128
...
\copy $TABLE_POKEMON(pokemon_id, uid, type_1, type_2, AT, DF, HP) from '$FILE_POKEMON' with CSV delimiter E'\t' NULL '';

-- Delete "dup" pokemons e.g. purified, fall_2019, etc.

-- PURIFIED
delete from $TABLE_POKEMON where uid ~ '_PURIFIED';

-- NORMAL of PURIFIED/SHADOW シャドウ化ありのポケモンは、NORMALを消して無印を残す
with a as (
    select B.uid from $TABLE_POKEMON A 
    join $TABLE_POKEMON B on B.uid=concat(substring(A.uid for position('_SHADOW' in A.uid)),'NORMAL') 
    where A.uid~'_SHADOW'
) delete from $TABLE_POKEMON where uid in (select * from a);

-- NORMAL of ALOLAN
with a as (
    select B.uid from $TABLE_POKEMON A 
    join $TABLE_POKEMON B on B.uid=concat(substring(A.uid for position('_ALOLA' in A.uid)),'NORMAL') 
    where A.uid~'_ALOLA'
) delete from $TABLE_POKEMON where uid in (select * from a);
...
with a as (
    select B.uid from $TABLE_POKEMON A 
    join $TABLE_POKEMON B on B.uid=concat(substring(A.uid for position('_GALARIAN' in A.uid)),'NORMAL') 
    where A.uid~'_GALARIAN'
) delete from $TABLE_POKEMON where uid in (select * from a);

-- Specific form only e.g. THUNDURUS_INCARNATE/THUNDURUS_THERIAN 無印が不要なパターン
delete from $TABLE_POKEMON 
where uid in ('CASTFORM','CHERRIM','DEOXYS','ROTOM','ARCEUS','KYUREM','GENESECT','MEOWSTIC','WORMADAM','GIRATINA','SHAYMIN','DARMANITAN','TORNADUS','THUNDURUS','LANDORUS','KELDEO','MELOETTA','FRILLISH','JELLICENT');

-- Only "Vanilla" form is required 無印以外はカラー等のバリエでしかないので戦闘計算に不要なもの
with a as (
    select B.uid from $TABLE_POKEMON A 
    join $TABLE_POKEMON B on position(A.uid in B.uid) > 0 and B.uid != A.uid 
    where A.uid in ('BURMY','SHELLOS','GASTRODON','BASCULIN','DEERLING','SAWSBUCK','PYROAR')
)delete from $TABLE_POKEMON where uid in (select * from a);

-- Cosplay イベントコスプレなどでVanillaだけを消す
with a as (
    select B.uid from $TABLE_POKEMON A 
    join $TABLE_POKEMON B on position(A.uid in B.uid) > 0 and B.uid != A.uid 
    where A.uid in ('CUBCHOO','GENGAR','PIKACHU')
)delete from $TABLE_POKEMON where uid in (select * from a);
-- こちらはShadow等も含むので指名消し
delete from $TABLE_POKEMON 
where uid in ('SABLEYE_COSTUME_2020','DELIBIRD_WINTER_2020','SLOWBRO_2021','SLOWPOKE_2020','BLASTOISE_COPY_2019','SQUIRTLE_FALL_2019','CHARIZARD_COPY_2019','CHARMANDER_FALL_2019','VENUSAUR_COPY_2019','BULBASAUR_FALL_2019');
...
select B.pokemon_id, B.uid, 'FRUSTRATION' as move, true::boolean as legacy 
from $TABLE_POKEMON A join $TABLE_POKEMON B on B.uid=A.uid
where A.uid~'_SHADOW';

insert into $TABLE_MON_CHARGE_RAW 
select B.pokemon_id, B.uid, 'RETURN' as move, true::boolean as legacy
from $TABLE_POKEMON A join $TABLE_POKEMON B on B.uid=substring(A.uid for position('_SHADOW' in A.uid)-1)
where A.uid~'_SHADOW';
